...
class VirtualGoalSettingsViewController: UITableViewController, SFSafariViewControllerDelegate {
    ...
    func loadHowToPage() {
        if let infoURL = URL(string: "https://github.com/DalyBartlett/ARAnalysis/wiki") {
    ...
    func loadMoreInfoPage() {
        if let infoURL = URL(string: "https://github.com/DalyBartlett/ARAnalysis/blob/master/README.md") {
            let safariVC = SFSafariViewController(url: infoURL)
            self.present(safariVC, animated: true, completion: nil)
            safariVC.delegate = self
        }
